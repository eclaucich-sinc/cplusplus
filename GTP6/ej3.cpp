...
json checkgeo(json &in, int N=100) {
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = i + 1; j < n; ++j) {
            ...
            // uniAB
            union_t uni;
            uni.shapes.push_back(std::move(shapes[i]));
            uni.shapes.push_back(std::move(shapes[j]));
            double uniAB = area(uni, N);
            shapes[i] = std::move(uni.shapes[0]);
            shapes[j] = std::move(uni.shapes[1]);
            pair_res["uniAB"] = uniAB;
            
            // intsAB
            intersection_t ints;
            ints.shapes.push_back(std::move(shapes[i]));
            ints.shapes.push_back(std::move(shapes[j]));
            double intsAB = area(ints, N);
            shapes[i] = std::move(ints.shapes[0]);
            shapes[j] = std::move(ints.shapes[1]);
            pair_res["intsAB"] = intsAB;
            
            // diffAB (A - B)
            difference_t diffAB_shape;
            diffAB_shape.shapes.push_back(std::move(shapes[i]));
            diffAB_shape.shapes.push_back(std::move(shapes[j]));
            double diffAB = area(diffAB_shape, N);
            shapes[i] = std::move(diffAB_shape.shapes[0]);
            shapes[j] = std::move(diffAB_shape.shapes[1]);
            pair_res["diffAB"] = diffAB;
            
            // diffBA (B - A)
            difference_t diffBA_shape;
            diffBA_shape.shapes.push_back(std::move(shapes[j])); // Primero B
            diffBA_shape.shapes.push_back(std::move(shapes[i])); // Luego A
            double diffBA = area(diffBA_shape, N);
            shapes[j] = std::move(diffBA_shape.shapes[0]);
            shapes[i] = std::move(diffBA_shape.shapes[1]);
...
int main(int argc, char* argv[]) {
    ...
    ofstream ofs("output_unique.json");
    if (ofs.is_open()) {
        ofs << out_json.dump(4) << endl;
        ofs.close();
        cout << "\n[Info] Resultados guardados exitosamente en 'output.json'." << endl;
    } else {
        cerr << "\n[Error] No se pudo crear el archivo 'output.json'." << endl;
    }
